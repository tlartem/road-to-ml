...
def _load_enriched(split_value, max_rows=None):
    ...
    Uses USING SAMPLE for random sampling (not LIMIT) so all time periods
    ...
    df = con.execute(f"""
        SELECT
            ...
            t.passenger_count, t.duration_min,
...
def run():
    ...
    with mlflow.start_run():
        ...
        _push_model_metrics("taxi-duration", metrics, len(train_df))
